    ...
    <header className="w-full border-b border-gray-200 bg-white sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4 flex-wrap justify-start">
          {logo.map((item, index) => (
            <PrismicNextLink key={index} field={item.link}>
              <PrismicNextImage
                field={item.image}
                className="w-28 md:w-[220px] h-[10px] object-contain cursor-pointer hover:opacity-80 transition"
              />
            </PrismicNextLink>
          ))}
        </div>
            ...
            <>
              ...
              {utilityIcons.map((item, index) => (
                <div key={index} className="relative">
                  ...
                  >
                    {item.icon_type === "Search" && <SearchIcon className="w-5 h-5 text-neutral-600" />}
                    {item.icon_type === "User" && <User className="w-5 h-5 text-neutral-600" />}
                    {item.icon_type === "ShoppingBag" && <ShoppingBag className="w-5 h-5 text-neutral-600" />}
                    {Array.isArray(item.title) ? <PrismicRichText field={item.title} /> :
      ...
      {isSupportPage && isMenuOpen && (
        <div className="md:hidden flex flex-col gap-2 px-4 py-3">
          <PrismicNextLink
            field={slice.primary.return_to_site}
            className="text-black font-bold px-4 py-3 hover:bg-gray-500 transition text-sm text-center"
          />
          <PrismicNextLink
            field={slice.primary.submit_a_request}
            className="bg-yellow-400 text-black font-bold px-4 py-3 hover:bg-gray-500 transition text-sm text-center"
          />
        </div>
      )}
