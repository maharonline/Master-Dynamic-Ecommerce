  ...
  return (
    <>
      ...
      <section className="relative w-full overflow-hidden h-[715px] sm:h-[750px] md:h-[850px] bg-black font-inter">
        ...
        {/* === Controls Section: DASH + DOTS + PAUSE (Like Your Image) === */}
        <div className="absolute bottom-10 right-0 -translate-x-1/2 flex items-center gap-4 z-40">
          ...
          <div className="flex items-center gap-3">
            {slides.map((_, index) => (
              ...
              >
                ...
                <span
                  className={`absolute rounded-full transition-all duration-300 ${current === index ? "w-5 h-5 bg-white/20 blur-md" : "w-0 h-0"
                    }`}
                />
                ...
                <span
                  className={`rounded-full transition-all duration-300 ${current === index
                    ? "w-[22px] h-1 bg-white opacity-90"
                    : "w-2 h-2 bg-white/50"
                    }
  `}
                />
          ...
          <button
            ...
            className="w-10 h-10 border border-white rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition"
          >
            {isPaused ? <PlayIcon size={18} /> : <Pause size={18} />}
          </button>
        </div>
